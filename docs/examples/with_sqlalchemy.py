...
def search_users_by_email(pattern: str) -> Result[list[User], str]:
    ...
    try:
        users = [u for u in _users_db.values() if pattern.lower() in u.email.lower()]
        return Ok(users)
    except Exception as e:
        return Err(f"database_error: {str(e)}")
...
def example_not_found():
    ...
    match result:
        ...
        case Err("user_not_found"):
            print("✓ User does not exist (handled gracefully)")
        case Err(error):
...
def get_user_email(user_id: int) -> Result[str, str]:
    ...
    return get_user_by_id(user_id).map(lambda user: user.email)
...
def example_bulk():
    ...
    result = get_active_users().map(lambda users: len(users))
...
def example_filter_results():
    ...
    result = search_users_by_email("alice").map(lambda users: [u.name for u in users])
    ...
    def transfer_contact(from_id: int, to_id: int) -> Result[dict, str]:
        ...
        return (
            ...
            .and_then(
                ...
                get_user_by_id(to_id).map(lambda _: user)  # Return original user
            )
            .map(lambda user: {"transferred_from": user.name, "status": "success"})
        )
...
def example_fallback():
    ...
    def get_user_or_default(user_id: int) -> Result[dict, str]:
        ...
        return (
            ...
            .or_else(lambda error: Ok(User(-1, "Guest", "guest@example.com", True)))
...
def example_api_pattern():
    ...
    def get_user_api(user_id: int) -> dict:
        ...
        match result:
            ...
            case Err(error):
                return {"status": "error", "message": error, "code": 400}
    ...
    print(
        f"✓ API Response (not found): {response['status']} (code: {response.get('code')})"
    )
